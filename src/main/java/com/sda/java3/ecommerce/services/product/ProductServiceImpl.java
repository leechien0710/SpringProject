...
public class ProductServiceImpl implements ProductService {
  ...
  public ProductServiceImpl(
      final ProductRepository clientRepository, final CategoryRepository categoryRepository) {
  ...
  public List<Product> getProducts(ProductListFilter filter) {
    if (filter != null && filter.getCategoryId() != null && !filter.getCategoryId().isEmpty()) {
      ...
      if (category != null) filter.setCategoryName(category.getName());
    }
  ...
  public void delete(UUID uuid) {
    ...
    if (byId.isPresent()) productRepository.deleteById(uuid);
  }
  ...
  public void update(UUID id, SaveProductRequest request) throws CustomException {
    Product product =
        productRepository
            .findById(id)
            .orElseThrow(() -> new CustomException(null, "Sản phẩm không tồn tại hoặc đã bị xóa."));
  ...
  public void createDummyProducts() {
    Category menCategory = Category.builder().name("MEN").build();
    categoryRepository.save(menCategory);

    Category womenCategory = Category.builder().name("WOMEN").build();
